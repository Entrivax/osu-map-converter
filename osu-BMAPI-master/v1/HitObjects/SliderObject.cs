...
namespace BMAPI.v1.HitObjects
{
    public class SliderObject : CircleObject
    {
        ...
        public double EndTime(Beatmap beatmap)
        {
            double beatLength = 0;
            double sliderVelocity = 1;
            for (int i = 0; i < beatmap.TimingPoints.Count; i++)
            {
                if (beatmap.TimingPoints[i].Time <= StartTime)
                {
                    if (beatmap.TimingPoints[i].InheritsBPM == false)
                    {
                        beatLength = beatmap.TimingPoints[i].BpmDelay;
                        sliderVelocity = 1;
                    }
                    else
                    {
                        sliderVelocity = -100 / beatmap.TimingPoints[i].BpmDelay;
                    }
                }
                else
                    break;
            ...
            return StartTime + (MaxPoints / (beatmap.SliderMultiplier * 100f * sliderVelocity)) * beatLength * RepeatCount;
        }
